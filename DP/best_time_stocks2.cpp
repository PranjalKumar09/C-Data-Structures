...
/* 
    Buying stocks, once maximum profit 2
        same like 1, but here can do infinitely but not more than one at same time

        first buy come then sell
            do in like Buy Sell Buy Sell ... 
        maximize profit
...
long f(int ind, bool buy ){
    ...
    long profit = 0;
    if (buy)
        profit = max(-arr[ind]+f(ind+1, 0), 0+f(ind+1, 1));
    else
...
long f2(int ind, bool buy ,vector<vector<long>> &dp ){
    ...
    long profit = 0;
    if (buy)
        profit = max(-arr[ind]+f2(ind+1, 0, dp), 0+f2(ind+1, 1, dp));
    else
