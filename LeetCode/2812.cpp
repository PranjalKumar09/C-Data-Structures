...
public:
    ...
    bool isValid(vector<vector<bool>>&visited, int i, int j){
        return  !(i < 0 || j < 0 || i == visited.size() || j == visited[0].size() || visited[i][j]);    }
    //============================================================================================================
    bool isSafe(vector<vector<int>>&distToTheif, int safeDist) //check the validity of safenessFactor
    {
        int n = distToTheif.size();
        queue<pair<int, int>>q;
        if (distToTheif[0][0] < safeDist || distToTheif[n-1][n-1] < safeDist) return false; // manhattan distance at starting left top must we lower than safedist and at 
        q.push({0, 0});
